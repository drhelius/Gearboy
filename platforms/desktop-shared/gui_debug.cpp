...
static void debug_window_audio(void)
{
    ...
    ImGui::TextColored(magenta, "CHANNEL 1 - TONE & SWEEP:");

    u8 value = apu_state.regs[0xFF10 - 0xFF10];
    ImGui::TextColored(cyan, " $FF10"); ImGui::SameLine();
    ImGui::TextColored(violet, "NR10"); ImGui::SameLine();
    ...
    ImGui::TextColored(magenta, "WAVE ($FF30 - $FF37):" );
    ...
    ImGui::TextColored(magenta, "WAVE ($FF38 - $FF3F):" );
...
static void debug_window_vram_tiles(void)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (window_hovered && (mouse_x >= 0.0f) && (mouse_x < width) && (mouse_y >= 0.0f) && (mouse_y < height))
        {
            ...
            draw_list->AddRect(ImVec2(p[i].x + (tile_x * spacing), p[i].y + (tile_y * spacing)), ImVec2(p[i].x + ((tile_x + 1) * spacing), p[i].y + ((tile_y + 1) * spacing)), ImColor(cyan), 2.0f, ImDrawFlags_RoundCornersAll, 2.0f);
