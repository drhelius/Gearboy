// Adapted from libretro slang-shaders handheld/gameboy pass3.
...
void main()
{
    ...
    out_color.a += texture(Source, clamp(tex_coord + vec2(0.0, texel.y), lower_bound, upper_bound)).a * weight1;
    out_color.a += texture(Source, clamp(tex_coord - vec2(0.0, texel.y), lower_bound, upper_bound)).a * weight1;
    out_color.a += texture(Source, clamp(tex_coord + vec2(0.0, texel.y * 2.0), lower_bound, upper_bound)).a * weight2;
    out_color.a += texture(Source, clamp(tex_coord - vec2(0.0, texel.y * 2.0), lower_bound, upper_bound)).a * weight2;
    out_color.a += texture(Source, clamp(tex_coord + vec2(0.0, texel.y * 3.0), lower_bound, upper_bound)).a * weight3;
    out_color.a += texture(Source, clamp(tex_coord - vec2(0.0, texel.y * 3.0), lower_bound, upper_bound)).a * weight3;
    out_color.a += texture(Source, clamp(tex_coord + vec2(0.0, texel.y * 4.0), lower_bound, upper_bound)).a * weight4;
    out_color.a += texture(Source, clamp(tex_coord - vec2(0.0, texel.y * 4.0), lower_bound, upper_bound)).a * weight4;
