...
const vec3 GAMEBOY_ORIGINAL_FOREGROUND = vec3(28.0 / 255.0, 106.0 / 255.0, 107.0 / 255.0);
...
vec3 foreground_palette_color(void)
{
    if (palette < 0.5)
        return GAMEBOY_ORIGINAL_FOREGROUND;
    if (palette < 1.5)
        return vec3(0.067, 0.098, 0.133);
    if (palette < 2.5)
        return vec3(0.125, 0.125, 0.125);
    if (palette < 3.5)
        return vec3(0.0, 0.0, 0.0);
    if (palette < 4.5)
        return vec3(0.114, 0.416, 0.420);
    if (palette < 5.5)
        return vec3(0.0, 0.325, 0.200);

    return vec3(0.0, 0.325, 0.314);
}
...
void main()
{
    ...
    vec3 foreground_color = foreground_palette_color();
